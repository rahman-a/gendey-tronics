...
const controlBlogView = id => async dispatch => {
    ...
    try {
        ...
        dispatch({type:constants.blogs.CONTROL_BLOG_VIEWS_SUCCESS, payload:data.seen})
    } catch (error) {
        dispatch({
...
const listBlogComments = id => async dispatch => {
    ...
    try {
        ...
        dispatch({type:constants.blogs.LIST_BLOG_COMMENTS_SUCCESS, payload:data.comments})
    } catch (error) {
        dispatch({
...
const addBlogComment = (id, comment) => async (dispatch, getState) => {
    ...
    try {
        ...
        console.log('Comment Action',data.comment);
        ...
        dispatch({type:constants.blogs.ADD_BLOG_COMMENT_SUCCESS, payload:data.message})
    } catch (error) {
        dispatch({
...
const deleteBlogComment = id => async (dispatch, getState) => {
    ...
    try {
        ...
        let {blog} = getState().blog 
        blog.comments = blog.comments.filter(comment => comment._id.toString() !== data.comment.toString())
        dispatch({type:constants.blogs.GET_BLOG_SUCCESS, payload:blog})
        dispatch({type:constants.blogs.DELETE_BLOG_COMMENT_SUCCESS, payload:data.message})
    } catch (error) {
        dispatch({
