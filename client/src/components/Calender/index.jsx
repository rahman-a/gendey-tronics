  ...
  const setMonthHandler = (action) => {
    if (action === 'prev') {
      if (parseInt(month) > 1) {
        ...
        setDate({
          ...date,
          month: dayjs(month).subtract(1, 'month').format('M'),
        })
      }
    } else if (action === 'next') {
  ...
  return (
    <div className={style.calender}>
      ...
      <div className={style.calender__time}>
        ...
        <div
          className={`${style.calender__time_inputs} ${
            lang === 'ar' ? style.calender__time_inputs_ar : ''
          }`}
        >
          <input
            type='number'
          ...
          />
          <span
            className={style.calender__time_period}
            onClick={setDayTimeHandler}
          >
            {dayTime}
          </span>
        </div>
        <button onClick={submitDateHandler}>
          {strings.product[lang].set_date}
        </button>
      </div>
      <div className={style.calender__date}>
        ...
        <div className={style.calender__date_inputs}>
          ...
          <input type='text' name='month' value={months[month - 1]} />
