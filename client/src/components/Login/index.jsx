...
const Login = () => {
  ...
  const {
    ...
    error: re_error,
    message,
  } = useSelector((state) => state.resetLinkPass)
  ...
  return (
    <>
      ...
      <Modal
        toggle={isPasswordForget}
        closeHandler={() => setIsPasswordForget(false)}
      >
        {errorMessage ? (
          <Alert variant='danger' dismissible onClose={clearResetPassAlerts}>
            {errorMessage}
          </Alert>
        ) : re_loading ? (
          <Loader size='10' center />
        ) : (
          message && (
            <Alert variant='success' dismissible onClose={clearResetPassAlerts}>
              {message}
            </Alert>
          )
        )}
        <form className={style.login__reset}>
          ...
          <button
            ...
            style={{ pointerEvents: re_loading ? 'none' : 'visible' }}
          >
            {strings.client[lang].reset_link}
          </button>
        </form>
      </Modal>
      <div className={style.login}>
        {errorMessage && (
          <Alert style={{ width: '70%', textAlign: 'center' }} variant='danger'>
            {errorMessage}
          </Alert>
        )}
        <form onSubmit={submitFormHandler}>
          ...
          <div className={style.login__options}>
            <div className={style.login__options_remember}>
              ...
              <label
                htmlFor='remember'
                style={{ marginRight: lang === 'ar' ? '0.5rem' : '0' }}
              >
                {strings.client[lang].remember_me}
              </label>
            </div>
            <p onClick={() => setIsPasswordForget(true)}>
              {strings.client[lang].pass_forget}
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <button
              ...
              className={style.login__submit}
            >
              {strings.client[lang].sign_in}
            </button>
            {loading && <Loader size='5' />}
          </div>
          <div className={style.login__alt}>
            <p>
              {strings.client[lang].or_signin} <span></span>
            </p>
            <div
              className={`${style.login__int} ${
                lang === 'ar' ? style.login__int_ar : ''
              }`}
            >
