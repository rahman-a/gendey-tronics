    ...
    return (
        <Template>
            <div className={`${style.blogs} ${lang === 'ar' ? style.blogs_ar :''}`}>
                <div className="container" style={{height:(loading || error) ? '35rem':'auto'}}>
                    {loading 
                    ? <Loader size='25' center custom={{color:'#d0ae0b'}}>
                        <p style={{fontSize:'2.5rem', fontWeight:'500', color:'#424242'}}>Loading Blogs....</p>
                     </Loader>
                    :error 
                    ? <Message type='error' size='5' message={error}/>
                    :blogs 
                    && <div className={style.blogs__wrapper}>
                        {blogs.map(blog => <BlogCard key={blog._id} blog={blog} lang={lang} strings={strings}/>)}
                    </div>}
                </div>
            </div>
        </Template>
