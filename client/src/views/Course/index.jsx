...
const Course = () => {
    ...
    return (
        <Template>
           { loading ? <div style={{padding:'21rem', marginBottom:'10rem'}}>
                <Loader size='20' center>
                    <p style={{fontSize:'1.5rem'}}>{strings.course[lang].course_load}</p>
                </Loader>
           </div> 
           : error ?<div style={{marginTop:'12rem', marginBottom:'2rem'}}>
           ...
           && <div className={style.course}>
               ...
               <CourseOverview data={{
                   ...
                   title:course.name,
                   date:new Date(course.enrollment.enrollmentDate).toLocaleDateString()
               }}/>
               <CourseInfoTab data={{
                    video:course.trailer,
                    price:course.price,
               ...
               `}>
                ...
                <div className={style.course__requirements}>
                    <h2 className={style.course__header}>{strings.course[lang].requirements}</h2>
                ...
                <div className={style.course__description}>
                    <h2 className={style.course__header}>{strings.course[lang].description}</h2>
                    ...
                    <button onClick={() => setShowContent(prev => !prev)}>
                        {showContent ? strings.course[lang].read_less: strings.course[lang].read_more}
                    </button>
                </div>
                {course.targets.length > 0 && <div className={style.course__learner}>
                    ...
                    style={{maxHeight: showLearners ? 'fit-content':'8rem'}}>                     
                        {
                            course.targets.map(target => (
                                <li className={style.course__learner_item}
                                key={uuidv4()}>{target}</li>
                            ))
                        }
                    </ul>
                ...
                <div className={style.course__content}>
                    <h2 className={style.course__header}>{strings.course[lang].course_content}</h2>
                    <div className={style.course__content_wrapper}
                    style={{direction:'ltr'}}>
                        {
                            course.chapters
                            .sort((a, b) => a.order - b.order)
                            .map(chapter => <Accordion 
                                chapter={chapter} 
                                key={chapter._id}
                                isPaid={course.isPaid}
                                lang={lang}/>)
                ...
                <div className={style.course__instructor}>
                    <h2 className={style.course__header}>{strings.course[lang].instructor}</h2>
                    <CourseInstructor instructor={course.instructor} lang={lang}/>
                </div>
                <div className={style.course__rating}>
                    <h2 className={style.course__header}>{strings.course[lang].feedback}</h2>
                    <CourseRating lang={lang} data={{
                        rating:course.reviewsData.averageNumericRating,
