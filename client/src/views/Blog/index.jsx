            ...
            <div className={style.blog}>
                <div className="container" style={{
                    ...
                    ?<Loader size='25' center custom={{color:'#d0ae0b'}}>
                        <p style={{fontSize:'2.5rem', fontWeight:'500', color:'#424242'}}>{strings.blog[lang].blog_load}</p>
                    </Loader>
                    : error 
                    ...
                    && <div className={style.blog__wrapper}>
                        ...
                        <p className={style.blog__date}>{`
                            By: ${blog.author.firstName} ${blog.author.lastName} on ${new Date(blog.createdAt).toDateString()} 
                        `}</p>
                        <figure>
                        ...
                        onClick={() => history.push('/login')}>{strings.blog[lang].log_comment}</h2>}
                        <div className={style.blog_comments}
                        style={{marginTop:'2rem'}}>
                            {
                            blog 
                            && blog.comments.length > 0
                            && blog.comments.map(comment => <BlogComment key={comment._id} comment={comment} />)
                            }
                        </div>
                    </div>
                    }
