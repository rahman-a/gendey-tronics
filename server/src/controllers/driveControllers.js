...
const getOauthClient = async () => {
    ...
    const credentials = JSON.parse(fs.readFileSync(path.join(__dirname, '../gcr.json'))).web 
...
const credential = _ => {
    const accessToken = JSON.parse(fs.readFileSync(path.join(__dirname, '../gact.json'))).tokens
    return accessToken
}
...
const downloadDriveFile = async (id) => {
    ...
    try {
        ...
        const response = await drive.permissions.list({fileId:id, fields:'permissions'})
        const permission = response.data.permissions[0]
        if(permission.role !== 'reader') {
          await  drive.permissions.create({fileId:id,
                requestBody:{
                role:'reader',
                type:'anyone'
                }  
...
const getGoogleAccessTokenByRefreshToken  = async _ => {
    const refresh_token = JSON.parse(fs.readFileSync(path.join(__dirname, '../gact.json'))).tokens.refresh_token 
    const {client_secret, client_id} = JSON.parse(fs.readFileSync(path.join(__dirname, '../gcr.json'))).web 
    ...
    return (await(await fetch(accessTokenUrl, accessTokenOption)).json()).access_token
}
...
export async function getResumableURI(req, res, next) {
    ...
    try {
        ...
        const parents = type === 'course' 
                        ? [GOOGLE_COURSES_FOLDER] 
                        : type === 'product' && [GOOGLE_PRODUCTS_FOLDER]
    
        const uris = JSON.parse(fs.readFileSync(path.join(__dirname, '../resumable_uri.json')))
        ...
        if(resumableURI) {
            ...
            fs.writeFileSync(path.join(__dirname, '../resumable_uri.json'), JSON.stringify(updatedUris))
    ...
    try {
        ...
        if(req.file) {
            ...
            if(errors.includes(response.status)) {
                ...
                throw new Error('service unavailable right now, please try again after while')
            } 
            else {
                if(response.status === 200 || response.status === 201) {
                   ...
                   if(type === 'course') {
                        const course = await Course.findById(id)
                        course.driveFile = course.driveFile.concat({link:metadata.id, part:parseInt(part)})
                        await course.save()
                   } else if (type === 'product') {
                        const product = await Product.findById(id)
                        product.driveFile = product.driveFile.concat({link:metadata.id, part:parseInt(part)})
                        await product.save()
                   }
                    ...
                    fs.writeFileSync(path.join(__dirname, '../resumable_uri.json'), JSON.stringify(uris))
...
export const resumePreviousUpload = async (req, res, next) => {
    ...
    try {
        ...
        if(!isResumableURI) {
            res.status(404)
            throw new Error('The upload link isn\'t found, maybe the download completed or you didn\'t start at all')
        }
        const options = {
            method: 'PUT',
            headers: {
