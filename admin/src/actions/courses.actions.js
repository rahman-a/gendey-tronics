...
const createChapter = (id, info) => async (dispatch, getState) => {
    ...
    try {
        ...
        if(course) {
            const copiedCourse = {...course}
            const filteredChapter = copiedCourse.chapters.filter(chapter => chapter._id !== data.chapter._id)
            console.log({copiedCourse});
            copiedCourse.chapters = [...filteredChapter, {...data.chapter, lessons:[]}]
            dispatch({
                type:constants.courses.GET_COURSE_SUCCESS,
                payload:copiedCourse
            })
...
const deleteChapter = id => async (dispatch, getState) => {
    ...
    try {
        ...
        if(course) {
            const copiedCourse = {...course}
            copiedCourse.chapters = copiedCourse.chapters.filter(chapter => chapter._id !== id) 
            dispatch({
                type:constants.courses.GET_COURSE_SUCCESS,
                payload:copiedCourse
            })
...
const createLesson = (id,info) => async (dispatch, getState) => {
    ...
    try {
        ...
        if(course) {
            ...
            copiedCourse.chapters.forEach(chapter => {
                if(chapter._id === id) {
                   const filteredLessons =  chapter.lessons.filter(lesson => lesson._id !== data.lesson._id)
...
const deleteLink = (id, link) => async(dispatch, getState) => {
    ...
    try {
        ...
        if(course) {
            const copiedCourse = {...course}
            copiedCourse.driveFile = copiedCourse.driveFile.filter(lk => lk._id.toString() !== link)
            dispatch({
                type:constants.courses.GET_COURSE_SUCCESS,
                payload:copiedCourse
            })
