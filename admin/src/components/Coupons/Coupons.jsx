...
import { Modal, Button, InputGroup, Form, Row, Col, Table, Badge } from 'react-bootstrap'
...
const Coupons = ({isCoupon, setIsCoupon}) => {
    ...
    <Modal show={isCoupon} onShow={listCoupons} onHide={onModalHide} size='lg'>
        <Modal.Header>
            <p>
                <span> <Coupon width='2em' height='2em'/> </span>
        ...
        <Modal.Body>
            <div className={style.coupons}>
                <div style={{marginBottom:'1rem', maxHeight:'35rem', overflowY:'auto'}}>
                    <Table striped bordered hover size="sm" className='text-center fs-5'>
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Code</th>
                        ...
                        <tbody>
                            {
                                coupons && coupons.map((coupon, idx) => (
                                    <tr>
                                        ...
                                        <td> <Button variant='danger' onClick={() => deleteCouponHandler(coupon._id)}> 
                                                    <Trash/> 
                                            </Button> 
                                        </td>
                                    </tr>
                ...
                <Row>
                    <InputGroup className='mb-3' style={{flexGrow:'3'}} as={Col}>
                        ...
                        <Form.Control onChange={({target:{value}}) => setCode(value)}/>
                    </InputGroup>
                    
                    <InputGroup className='mb-3' style={{flexGrow:'2'}} as={Col}>
                        <InputGroup.Text> discount value % </InputGroup.Text>
                        <Form.Control type='number' onChange={({target:{value}}) => setDiscount(value)}/>
                    </InputGroup>

                    <InputGroup className='mb-3' style={{flexGrow:'2'}} as={Col}>
                        <InputGroup.Text> limited to </InputGroup.Text>
                        <Form.Control type='number' onChange={({target:{value}}) => setLimits(value)}/>
                    </InputGroup>

                    <InputGroup className='mb-3' style={{flexGrow:'3'}} as={Col}>
                        <InputGroup.Text> expired at </InputGroup.Text>
                        <Form.Control type='date' onChange={({target:{value}}) => setExpiryAt(value)}/>
                    </InputGroup>
        ...
        <Modal.Footer>
            ...
            <Button variant='danger' onClick={onModalHide}> Cancel </Button>
        </Modal.Footer>
    </Modal>
  )
}
