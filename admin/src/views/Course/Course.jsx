...
import {BackButton, SideAlert, Loader, CourseRating, Instructor} from '../../components'
...
const Course = () => {
    ...
    const {loading:image_loading, error:image_error, message} = useSelector(state => state.updateCourseImage)
    const {loading:publish_loading, error:publish_error} = useSelector(state => state.togglePublish)
    ...
    const createNewLesson = (lesson, chapterId) => {
        ...
        dispatch({type:constants.courses.GET_COURSE_SUCCESS, payload:updatedCourse})
    }
    ...
    const removeChapter = id => {
        const updatedCourse = {...course}
        updatedCourse.chapters = updatedCourse.chapters.filter(chapter => chapter._id !== id)
        dispatch({type:constants.courses.GET_COURSE_SUCCESS, payload:updatedCourse})
    }
                     ...
                     <>
                        <div className={style.course__header}> 
                            ...
                            <span  className={style.course__header_date}> 
                                created At: {new Date(course.createdAt).toLocaleDateString('en-US', dateOptions)} 
                            </span>
                        </div>
                        <div className={style.course__body}> 
                            <div className={style.course__segment}>
                                ...
                                <figure className={style.course__image}>
                                    <div className={style.course__image_backdrop}
                                    ...
                                    <img src={`/api/images/${course.image}`} alt={course.name} />
                                </figure>
                            </div>
                                ...
                                <div className={style.course__segment}>
                                    <Requirements data={course.requirements} courseId={course._id}/>
                                ...
                                <div className={style.course__segment}>
                                        ...
                                        <Accordion defaultActiveKey={['0']} alwaysOpen>
                                            <TransitionGroup>
                                            {
                                                course.chapters.map((chapter, idx) => (
                                                    <CSSTransition key={chapter._id} timeout={1500} classNames='chapter'>
                                                        <Chapter 
                                                        courseId={course._id}
                                                        data={chapter}
                                    ...
                                    <div style={{marginTop:'4rem'}}>
                                        <h1 className={style.course__level}> Course Instructor </h1>
