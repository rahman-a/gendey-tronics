  ...
  return <>
        <Modal show={confirmDelete} onHide={() => setConfirmDelete(false)}>
          <Modal.Body>
            ...
            <p className={style.products__confirmDelete}>Do You Really want to delete the product permanently.</p> 
          </Modal.Body>
          <Modal.Footer>
...

    {
        <>
            ...
            <td>{product.name}</td>
            <td>
                <img className='row-photo' src={`/api/images/${product.image}`} alt={product.name} />
            </td>
            <td>{product.type}</td>
            <td>{product.price}</td>
            <td>{product.quantity}</td>
            <td className={style.products__action}>
                <span className={style.products__delete}
                style={{position:'relative'}}>
                    {
                    isDeleting 
                    ? <Loader size='4' center options={{animation:'border'}}/>
                    : <span style={{cursor:'pointer'}} onClick={() => initiateDeleteProcess(product._id)}> 
                        <Times/> 
                    </span> 
                    }
                </span> 
                <span className={style.products__edit}
                style={{position:'relative'}}>
                    <span style={{cursor:'pointer'}} onClick={() => history.push(`/products/${product._id}`)}> 
                        <Edit/> 
                    </span> 
                </span> 
            </td>
