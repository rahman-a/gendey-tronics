...
const UploadAction = ({ isUpload, setIsUpload }) => {
  ...
  const {
    ...
    range,
  } = useSelector((state) => state.resumeFile)
  ...
  const resumeFileHandler = (_) => {
    if (file && range) {
      ...
      setProgressText(
        `uploading in progress ${parseFloat(progressValue).toFixed(1)}%`
      )
  ...
  useEffect(() => {
    if (uploaded) {
      ...
      setProgressText(
        `uploading in progress ${parseFloat(progress).toFixed(1)}%`
      )
  ...
  return (
    <Modal show={isUpload}>
        ...
        <div className={style.drive__upload}>
          {!isUploading && (
            <div className={style.drive__upload_type}>
              <Button variant='info' onClick={() => setAsset('product')}>
                Product
              </Button>
              <Button
                className='mx-3'
      ...
      <Modal.Footer>
        <Button variant='danger' onClick={resetUploadHandler}>
          Close
        </Button>
      </Modal.Footer>
    </Modal>
