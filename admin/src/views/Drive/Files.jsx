...
const Files = ({files}) => {
  ...
  const {loading, error, files:filteredFiles} = useSelector(state => state.searchFiles)
        ...
        <div className={style.drive__content_wrapper}>
              ...
              <div className={style.drive__content_files}>
                <div className={style.drive__content_actions}>
                    <div className={style.drive__content_search}>
                        <Form.Group className="mb-3" controlId="formBasicType">
                            <Form.Select name='type' size='lg' onChange={(e) => filterFilesByTypeHandler(e.target.value)}>
                                <option value=''>search by type...</option>
                                <option value="products">Products</option>
                                <option value="courses">Courses</option>
                            </Form.Select>
                        </Form.Group>
                        <Form.Group className="mb-3" controlId="formBasicType" onKeyDown={filterFilesByNameHandler}>
                            <Form.Control
                        ...
                        <Form.Group className="mb-3" controlId="formBasicType">
                            <Button variant='dark' onClick={resetSearchHandler}>reset..</Button>
                        </Form.Group>
                    </div>
                    <div className={style.drive__content_layout}>
                        <Button variant='light' onClick={() => setIsHorizontal(false)}> <GridMenu/> </Button>
                        <Button variant='light' onClick={() => setIsHorizontal(true)}> <StackMenu/> </Button>
                    </div>
                </div> 
               {files.length === 0 
               &&  <div className={style.drive__noFiles}> 
                ...
                <div className={`
                ${style.drive__content_container} ${isHorizontal ? style.drive__content_container_horizontal : ''}`}>
                    {
                        loading ? 
                        <div className={style.drive__noFiles} 
                        ...
                        : driveFiles.map( file => <File key={file.id} file={file} isHorizontal={isHorizontal}/>)
                    }
                </div>
               {/* { files.length > 0 
