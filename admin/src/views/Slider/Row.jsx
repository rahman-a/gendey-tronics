...
const Row = ({slide, idx}) => {
    ...
    <>
       <Modal show={confirmDelete} onHide={() => setConfirmDelete(false)}>
         <Modal.Body>
           ...
           <p className={style.sliders__confirmDelete}>Do You Really want to delete the slider permanently.</p> 
         </Modal.Body>
         <Modal.Footer>
       ...
       <Modal show={isImage} onHide={() => setIsImage(false)} fullscreen={true}>
            ...
            <Modal.Body>
                ...
                src={`/api/images/${slide.image}`} alt='slider'/>
            </Modal.Body>
            <Modal.Footer>
                <Button style={{marginRight:'2rem'}} variant="secondary" size='lg' onClick={() => setIsImage(false)}>Close</Button>
            </Modal.Footer>
       </Modal>

   {
       <>
           ...
           <td> {slide.subHeader} </td>
           
           <td style={{padding:'0'}}>
               <p className={style.sliders__image}> 
                   <span onClick={() => setIsImage(true)}><Eye/></span> 
           ...
           <td> { new Date(slide.createdAt).toLocaleDateString('en-US', dateOptions) } </td>
           <td className={style.sliders__action}>
               <span className={style.sliders__delete}
