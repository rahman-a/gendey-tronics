...
const OrderItems = ({itemsToggle, setItemsToggle, items}) => {
  ...
  return <Modal show={itemsToggle} onHide={() => setItemsToggle(false)}>
      <Modal.Header>
            <div className={style.orders__items_header}>
                <h2>Order Items List</h2>
                <p> Order Id: <i> {items._id} </i> </p>
            </div>
      </Modal.Header>
      <Modal.Body>
            <div className={style.orders__items_list}>
                <Accordion>
                    {
                        items.data.map(item => (
      ...
      <Modal.Footer>
            <Button onClick={() => setItemsToggle(false)} variant='danger' size='lg'> Close </Button>
      </Modal.Footer>
  </Modal>;
};
