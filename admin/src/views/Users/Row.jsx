...
import {Times, Check, Copy} from '../../icons'
  ...
  return <>
        <Modal show={confirmDelete} onHide={() => setConfirmDelete(false)}>
          <Modal.Body>
            ...
            <p className={style.users__confirmDelete}>Do You Really want to delete the user permanently.</p> 
          </Modal.Body>
          <Modal.Footer>
...

    {
        <>
            ...
            <td style={{display:'grid', placeItems:'center'}}>
                <span className={style.users__delete}
                style={{position:'relative'}}>
                    {
                    isDeleting 
                    ? <Loader size='4' center options={{animation:'border'}}/>
                    : <span style={{cursor:'pointer'}} onClick={() => initiateDeleteProcess(user._id)}> 
                        <Times/> 
                    </span> 
                    }
                </span> 
