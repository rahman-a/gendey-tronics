...
const Message = ({ email }) => {
  ...
  return (
    <div className={style.support__messages_item} onClick={loadEmailHandler}>
      ...
      <div className={style.support__messages_item_content}>
        <p style={{ fontWeight: email.isRead ? '400' : '900' }}>
          {' '}
          {email.from}{' '}
        </p>
      ...
      <div className={style.support__messages_item_state}>
        <span>{msToTime(email.createdAt)}</span>
        <span style={{ backgroundColor: email.isRead ? '' : '#F8C600' }}>
          {' '}
        </span>
      </div>
    </div>
