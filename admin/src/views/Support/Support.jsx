...
const Support = () => {
  ...
  const {isLoading, error, inbox, emails} = useSelector(state => state.listEmails)
  const {isLoading: isLoadingEmail, error: errorEmail, email} = useSelector(state => state.getEmail)
  const {id} = useParams()
      ...
      />
        ...
        <div className={style.support__actions}>
          <div className={style.support__actions_new}>
              <span onClick={() => setIsNewEmail(true)}> <EnvelopOpen/> </span>
          </div>
          <Sidebar inbox={inbox}/>
        </div>
          ...
          <div className={style.support__messages_items}>
              
              {
                ...
                : emails && emails.map(email => (
                  <Message key={email._id} email={email}/>
                ))
              }
          </div>
        </div>
