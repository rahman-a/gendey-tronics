  ...
  return (
    <>
      ...
      <Modal show={confirmDelete} onHide={() => setConfirmDelete(false)}>
        <Modal.Body>
          <p
            className={style.calls__confirmDelete}
            style={{ fontSize: '2rem' }}
          >
            {' '}
            Are you sure?
          ...
          <p className={style.calls__confirmDelete}>
            Do You Really want to delete the call permanently.
          </p>
        </Modal.Body>
        <Modal.Footer>
          <Button
...

      {
        <>
          ...
          <td className={style.calls__name}>
            {call.product ? (
              <span onClick={navigateToProduct}>{call.product.name}</span>
            ) : (
          ...
          <td className={style.calls__method}>
            {call.method === 'phone' ? (
              <Badge bg='danger' title='Phone'>
                {' '}
                <Phone />{' '}
              </Badge>
            ) : call.method === 'whats' ? (
              <Badge bg='success' title='whats up'>
                {' '}
                <Whatsapp />{' '}
              </Badge>
            ) : (
              call.method === 'zoom' && (
                <Badge bg='primary' title='Zoom'>
                  {' '}
                  <Zoom />{' '}
                </Badge>
              )
          ...
          <td className={style.calls__action}>
            ...
            >
              {isToggling ? (
                ...
                <span
                  title='mark call as done'
                  style={{ cursor: 'pointer' }}
                  onClick={toggleTheCall}
                >
                  <HandshakeSlash />
                </span>
              )}
