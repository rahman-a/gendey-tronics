  ...
  return <>
        ...
        <Modal show={confirmDelete} onHide={() => setConfirmDelete(false)}>
          <Modal.Body>
            ...
            <p className={style.courses__confirmDelete}>Do You Really want to delete the course permanently.</p> 
          </Modal.Body>
          <Modal.Footer>
            <Button variant="info" size='lg' onClick={() => setConfirmDelete(false)}>
              NO, Don't Delete
            </Button>
            <Button variant="danger" size='lg' onClick={confirmDeleteProductHandler}>
              YES, Delete
            </Button>
          </Modal.Footer>
        </Modal>

    {
        <>
            ...
            <td>{course.name}</td>
            <td>
                <img className='row-photo' src={`/api/images/${course.image}`} alt={course.name} />
            </td>
            
            <td style={{padding:course.description ? '0' :'2.5rem 0'}}>
                {course.description
                ? <p className={style.courses__description}> 
                    <span onClick={() => setIsDescriptionOn(true)}><Reader/></span> 
            ...
            <td className={course.students > 0 && style.courses__enroll}>
                {
                    <span onClick={showCourseEnrollments}> 
                        {course.students} 
                    </span>  
                }
            </td>
            ...
            <td>
              {
                msToTime(new Date().getTime() - new Date(course.updatedAt).getTime()) + ' ago'
              }
            </td>
            
            <td className={style.courses__action}>
                <span className={style.courses__delete}
                ...
                <span className={style.courses__edit}
                style={{position:'relative'}}>
                    <span style={{cursor:'pointer'}} onClick={() => history.push(`/courses/${course._id}`)}> 
                        <Edit/> 
                    </span> 
                </span> 
            </td>
